...
async function ghFetch(path, token) {
  ...
  if (res.status === 404) {
    throw new GithubError(
      token ? "Repository not found" : "Repository not found (or private — sign in to access it)",
      404
    );
  }
  ...
  if (res.status === 403) {
    ...
    if (remaining === "0") {
      throw new GithubError(
        token ? "GitHub rate limit hit — try again later" : "GitHub rate limit hit — sign in for a higher limit",
        403
      );
    }
